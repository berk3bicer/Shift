...
// Panelin eski `.font-brand` span'inin yerini alır → site (marketing) ile birebir aynı marka.
//
// Neden: (a) her cihazda birebir aynı görünür (Switzer FOUT/eksikliği etkilemez), (b) web/'in
// de outline'a geçmesiyle Switzer'in son gerçek kullanıcısı gitti → font iki taraftan silindi.
//
// PATH'LER marketing/components/Wordmark.tsx ile BİREBİR AYNI STRING — Switzer-Extrabold'dan
// fonttools ile çıkarıldı, "Shiftle" tek kesintisiz akış (kerning fontun kendi metriğinde).
// i-noktası (tittle) ayrı kontur → amber. DEĞİŞİRSE İKİSİ BİRLİKTE değişir (gen-icons.mjs de
// kendi kopyasını taşır; üç kopyanın tek-kaynağa indirgenmesi ayrı bir refactor turu).
//
// viewBox yüksekliği 1em (1000 upm) → `height:1em` cap-height'ı 0.694em render eder. Gövde
// `currentColor` (className'deki text miras — panel açık zemin + koyu sidebar ikisinde de doğru);
// nokta sabit amber.
const BODY_D =
